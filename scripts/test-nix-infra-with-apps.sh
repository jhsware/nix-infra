...
echo "---"
$NIX_INFRA deploy-apps -d $WORK_DIR --batch --env="$WORK_DIR/.env" \
  --target="service001 worker001"
$NIX_INFRA cmd -d $WORK_DIR --target="service001 worker001" "nixos-rebuild switch --fast"
$NIX_INFRA cmd -d $WORK_DIR --target="service001 worker001" "systemctl restart confd"
