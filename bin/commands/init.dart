...
class InitCommand extends Command {
  ...
  final name = 'init';
  @override
  final description = 'Initialize new infrastructure with SSH keys and optional certificate authority';
  ...
  void run() async {
    ...
    if (!noCertAuth) {
      ...
      final pwdCa = env['CA_PASS'] ?? readPassword(ReadPasswordEnum.caRoot, batch);
      final pwdCaInt = env['INTERMEDIATE_CA_PASS'] ?? 
        readPassword(ReadPasswordEnum.caIntermediate, batch);
      final certEmail = env['CERT_EMAIL'] ?? readInput('certificate e-mail', batch);
