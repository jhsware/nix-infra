...
- Get hash for fetchFromGitHub (use GitHub archive URLs)
...
''',
  ...
  Future<CallToolResult> callback({args, extra}) async {
    ...
    switch (operation) {
      case 'hash':
        return await _hashUrl(url, format);
      case 'verify':
        return await _verifyUrl(url, expectedHash);
      case 'convert':
  ...
  Future<CallToolResult> _hashUrl(String? url, String format) async {
    ...
    try {
      ...
      switch (format) {
        case 'nix32':
          hashResult = NixHash.sha256Nix32(bytes);
          break;
        case 'sri':
          hashResult = NixHash.sha256Sri(bytes);
          break;
        case 'hex':
          hashResult = NixHash.sha256Hex(bytes);
          break;
        default:
  ...
  Future<CallToolResult> _verifyUrl(String? url, String? expectedHash) async {
    if (url == null || url.isEmpty) {
      return _errorResult('Missing required parameter: url');
    }
    ...
    try {
      ...
      final nix32 = NixHash.sha256Nix32(bytes);
      final sri = NixHash.sha256Sri(bytes);
      final hex = NixHash.sha256Hex(bytes);
  ...
  CallToolResult _convertHash(String? hash, String? fromFormat, String toFormat) {
    ...
    try {
      ...
      switch (fromFormat) {
        ...
        case 'sri':
          ...
          final base64Part = hash.startsWith('sha256-') 
              ? hash.substring(7) 
              : hash;
